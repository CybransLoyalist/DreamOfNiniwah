...
namespace Assets.Scripts
{
    public class MountainBuilder
    {
        ...
        public static Dictionary<Vector2Int, float> BuildHollow(int x, int y, int bottomDepth, int ringWidth, int maxX, int maxZ)
        {
            var result = new Dictionary<Vector2Int, float>();
            ...
            foreach (var tile in circleAroundPeak)
            {
                result.Add(tile, bottomDepth);
            }


            int level = bottomDepth + 1;
            var ringBeginning = circleAroundPeakRadius + 1;
